...
// pointer near the end does not snap back onto the last whole step.
export function snapSliderValue(
  ...
  // Neither case has a grid to walk, and bailing keeps the division below
  // away from zero.
...
export interface SliderOptions {
  ...
  disabled?: boolean
  "aria-label"?: string
  // Announced instead of the raw number. Pass one when the value carries
  // a unit or suffix ("72.5 kg", "35%"); a bare number needs none.
  formatValueText?: (value: number) => string
}

// Value and input plumbing shared by slider designs. Visuals and motion
// live in the component; this only owns the number.
export function useSlider({
  ...
  // Collapse inverted ranges and non-positive steps here so percent,
  // ticks and the keyboard maths never divide by zero or walk a NaN grid.
  const lo = min
  ...
  const onPointerDown = useCallback(
    (event: PointerEvent<HTMLDivElement>) => {
      if (disabled) return
      // Test DOMs and older browsers omit pointer capture.
  ...
  const endDrag = useCallback((event: PointerEvent<HTMLDivElement>) => {
    // Releasing without capture throws.
