...
export function MessageScroller({
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => {
      ...
      // Streaming text resizes the content many times a second. Re-issuing a
      // "smooth" scrollTo on every one restarts the browser's easing curve
      // mid-flight, which reads as stutter, so pin to the bottom every frame
      // instead. At that update rate an instant jump reads as smooth.
      if (frameRef.current) cancelAnimationFrame(frameRef.current)
      frameRef.current = requestAnimationFrame(() => scrollToEnd("auto"))
    })
