...
export function AnimatedSidebarProvider({
  ...
  const [width, setWidthState] = useState(getStoredSidebarWidth)
  ...
  function AnimatedSidebar(
    ...
    return (
      <motion.aside
        ...
        animate={{ width }}
        transition={
          context.reduce || context.resizing
            ? { duration: 0 }
...
>(function AnimatedSidebarRail(
  ...
  const canResize = !panel.collapsed && panel.collapsible !== "none"

  const handlePointerDown = (event: React.PointerEvent<HTMLButtonElement>) => {
    onPointerDown?.(event)
    if (event.defaultPrevented || !canResize || event.button !== 0) return
    draggingRef.current = true
    draggedRef.current = false
    startXRef.current = event.clientX
    startWidthRef.current = context.width
  ...
  const handlePointerMove = (event: React.PointerEvent<HTMLButtonElement>) => {
    onPointerMove?.(event)
    if (event.defaultPrevented || !draggingRef.current) return
    const delta = event.clientX - startXRef.current
    const directional = panel.side === "left" ? delta : -delta
    if (Math.abs(directional) > 2) draggedRef.current = true
    context.setWidth(startWidthRef.current + directional)
  }
  ...
  return (
    <button
      ...
      aria-label={props["aria-label"] ?? "Toggle sidebar"}
      title={canResize ? "Drag to resize, click to toggle" : "Toggle sidebar"}
      tabIndex={-1}
...
>(function AnimatedSidebarFooter({ className, ...props }, forwardedRef) {
  return (
    <div
      ...
      className={cn(
        "relative flex shrink-0 flex-col gap-2 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]",
        // fading blur scrim instead of border-t
        "before:pointer-events-none before:absolute before:inset-x-0 before:-top-6 before:bottom-0",
        "before:backdrop-blur-md before:content-['']",
        "before:mask-[linear-gradient(to_bottom,transparent,black_1.5rem)]",
        "before:[-webkit-mask-image:linear-gradient(to_bottom,transparent,black_1.5rem)]",
        className
      )}
    >
      <div className="relative z-10 flex flex-col gap-2">{props.children}</div>
    </div>
  )
})
...
export interface AnimatedSidebarMenuSubButtonProps {
  ...
  icon?: ReactNode
  badge?: ReactNode
  // Second line under the label, e.g. a live "Working · 12s" status. Grows
  // the row from a single line to a two-line card so there's room for it.
  meta?: ReactNode
  href?: string
  isActive?: boolean
  disabled?: boolean
...
export function AnimatedSidebarMenuSubButton({
  ...
  const interactiveClassName = cn(
    "flex w-full min-w-0 gap-2 rounded-lg px-2 text-left text-xs outline-none",
    meta ? "min-h-11 items-start py-2" : "min-h-8 items-center",
...
export function AnimatedSidebarMenuButton({
  ...
  const content = (
    <>
      ...
      >
        ...
        {meta ? (
          <span className="mt-0.5 block truncate text-[11px] font-normal text-muted-foreground/70">
            {meta}
          </span>
        ) : null}
      </motion.span>
  ...
  const interactiveClassName = cn(
    "relative flex w-full min-w-0 gap-2.5 overflow-hidden rounded-xl px-3 text-left text-sm font-medium outline-none",
    meta ? "min-h-12 items-start py-2" : "min-h-9 items-center",
